...
# 1) Docstring decorators: override as no-ops to ignore old signatures.
...
# 2) Conv1D: removed from newer transformers; provide a lightweight replacement.
try:
...
class Attention(nn.Module):
    ...
    def prune_heads(self, heads):
        # Head pruning is not required for our use case; keep as no-op
        # for compatibility with older HuggingFace APIs.
        return
...
class GPT2Model(GPT2PreTrainedModel):
    def __init__(self, config):
        ...
        # HuggingFace GPT2Config newer versions no longer expose n_ctx; fall back to n_positions.
