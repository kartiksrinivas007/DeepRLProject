#!/bin/bash
# Run the walker REINFORCE-baseline test (5 iters) locally for all configs sequentially.
...
for IDX in "${!MODEL_NAMES[@]}"; do
  (
    ...
    RUN_DIR="sbatch_scripts/logs/reinforce_online_baseline_walker_test_local/critic_${CRITIC_COEF}/tau_${TAU}/no_buffer/model_${IDX}/rollouts_${NUM_ROLLOUTS_PER_ITER}_updates_${NUM_UPDATES_PER_ITER}/iters_${MAX_TRAIN_ITERS}"
    PLOT_DIR="${RUN_DIR}/plots"
    MODEL_DIR="models/reinforce_online_baseline_walker_test_local/critic_${CRITIC_COEF}/tau_${TAU}/no_buffer/model_${IDX}/rollouts_${NUM_ROLLOUTS_PER_ITER}_updates_${NUM_UPDATES_PER_ITER}/iters_${MAX_TRAIN_ITERS}"
    ...
    echo "------------------------------------------------------------"
    echo "Walker REINFORCE baseline TEST (local, background) idx=${IDX}"
...
echo "Launched ${#pids[@]} walker REINFORCE baseline TEST jobs in background on the same GPU."
echo "PIDs: ${pids[*]}"
echo "Use 'wait' to block until all finish, or monitor the log files directly."
echo "Background processes: ${pids[*]}"
# Do not wait here so the script returns immediately.
